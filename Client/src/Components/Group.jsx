    ...
    return (
        <div className='m-4'>
            ...
            <div className='bg-white rounded-md mt-4 p-4 flex flex-wrap gap-5 '>
                ...
                {!isLoading && <div className="card w-60 bg-base-100 shadow-xl">
                    <figure><img className='rounded-full w-44 h-44' src={groups[0]?.logo !== null ? groups[0]?.logo : "https://morganwhite.com/assets/img/icon-group.png"} alt={groups.logo} /></figure>
                    <div className="card-body">
                        <h2 className="card-title">{groups[0]?.email}</h2>
                        <p>{groups[0]?.name}</p>
                        <div className="card-actions justify-end">
                            <NavLink to={`/groupChat/${groups[0]?.id}`} className="btn bg-navBgColor">Chat</NavLink>
                        </div>
                    </div>
                </div>
                }
