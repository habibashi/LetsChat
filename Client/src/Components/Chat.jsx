...
import { createMessage, getRoom, reset } from '../features/Chat/PeopleSlice';
import { useParams } from 'react-router-dom';

const Chat = () => {
    const { user } = useSelector((state) => state.auth);
    const params = useParams();
    const { room, isLoading } = useSelector((state) => state.people);
        ...
        <div className="bg-white" style={{ height: "90vh" }}>
            <div className='border gap-3 bg-bgHeader flex items-center justify-between' style={{ height: "10vh" }}>
                <div className='flex items-center gap-3'>
                    <img className='w-11 h-11 rounded-full ml-4' src={room.users && room.users.find((u) => u.id !== user.id).photo !== null ? room.users.find((u) => u.id !== user.id).photo : "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png"} alt="img" />
                    <p className='font-bold'>{room.users && room.users.find((u) => u.id !== user.id).name}</p>
                </div>
            ...
            <div className='overflow-y-auto' style={{ height: "72vh" }}>
                ...
                {!isLoading && room && room.messages && room.messages.map(message => (
                    <li
                        key={message.id}
